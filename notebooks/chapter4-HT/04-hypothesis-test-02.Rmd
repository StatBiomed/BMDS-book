...
Here, we can check the probability mass function, and a close approximation with
binomial distribution.

```{r fig4-hyper, out.width = '60%'}
# plot the probability mass function of hypergeomatric distribution
# x need be 39<=x<=52, as n_male and n_female are >= 0.

x = seq(35, 55)
pmf_hyper = dhyper(x, m = 52, n = 48, k = 87)
pmf_binom = dbinom(x, 52, p=87/100)
df_hyper = data.frame(x=x, pmf=pmf_hyper, extreme=(x >= 48 | x <= 43))
df_binom = data.frame(x=x, pmf=pmf_binom, extreme=NA)
df = rbind(df_hyper, df_binom)

library(ggplot2)

fig_hyper = ggplot(df, aes(x=x)) + 
  geom_point(aes(y=pmf, color=extreme)) +
  scale_color_manual(values = c("black", "red", "grey"), 
                     labels = c('non-extreme', 'extreme', 'binomial')) +
  xlab('Number of male_right-handed') + 
  ylab('Probability Mass Function') +
  ggtitle('Hypergeometric distribution as Null')
