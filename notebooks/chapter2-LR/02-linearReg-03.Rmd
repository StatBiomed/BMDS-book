...
This means that all set of the features are useless versus that some are useful.
The statistic that is used to test this hypothesis is called the **F-statistic** 
and is defined as a ratio of the variance explained by the model to the variance 
that is left unexplained (noise), both are per degree of freedom (DF), as follows:

$$  F\text{-statistic} = \text{Fisher statistic}=  \frac{ \text{Expained Variance}  / \text{Model DF }}{ \text{Unexplained variance} / \text{Error DF} }
$$
In linear regression, we can write it explicitly as follows,
$$  F\text{-statistic} =  \frac{ SSR/ p }{ SSE/(n-p-1)}
$$
...
Moreover, one may use the F statistic for assessing a sub-group of features, say
$q$ last features without loss of generality. We can set up the hypothesis 
testing as follows,
- Null hypothesis: $\beta_{p-q+1} = \beta_{p-q+2} = ... \beta_{p} = 0}
- Alternative hypothesis: at least one of the coefficient is not zero.

Assuming, with the null hypothesis by dropping all the $q$ features, the new 
Sum of Squared Error is $SSE_{sub}$. Then we can have the $F$-statistic as follow,
$$  F\text{-statistic} =  \frac{ (SSE_{sub} - SSE_{full})/ q }{ SSE_{full}/(n-p-1)}
$$

You may realize the default $F$-statistic by dropping all features is a special 
case, as when not using any feature, $SSE_{sub}=SST$, as the sum of square total.
