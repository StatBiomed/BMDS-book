...
* R is the backbone of R programming. Once R is installed, you can use it 
  via its built-in `R Console` (self-contained), terminal or any third-party
  integrated development environment (IDE), e.g., RStudio.
  
* RStudio is a multi-faceted and user-friendly IDE that can make R programming and 
...
- Choose the ``base`` and then Click on the ``Download R 4.5.1 for Windows``
...
- Once the download is finished, you will obtain a file named 
  ``R-4.5.1-win.exe`` or similar depending on the version that you download.
...
- Click on the file containing the latest version of R under the 
  **Latest release**.
- Save the ``R-4.5.1-**.pkg`` file, double-click it to open, and follow the installation 
  instructions. 
  

> <span style="color: red;"> **Note**,
  there are two versions of the .pkg installation file according to 
  the CPU model: Intel Macs (Intel-based) or M1/M2/.. Macs (ARM-based). 
  **Please choose accordingly**. </span>
...
- Go to https://posit.co/download/rstudio-desktop/#download. We are 
...
RStudio cloud (https://posit.cloud; previously known as https://rstudio.cloud/). You can explore directly from their 
website.
